        ...
        <ul className="hidden lg:flex items-center justify-around w-110 h-full">
          {navLinks?.map((item, idx) => (
            <Link
              to={item.to}
              key={idx}
              className="relative font-medium after:absolute after:w-0 after:h-1 after:rounded-full  after:bg-accent-raspberry after:opacity-0 hover:after:w-[80%] hover:after:opacity-100 after:transition-all after:duration-300"
            >
              <li>{item.text}</li>
            </Link>
          ))}
